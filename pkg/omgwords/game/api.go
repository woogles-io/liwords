...
// NewGame creates a new GameRecord. The playerinfo array contains
// the players, which must be in order of who goes first!
func NewGame(cfg *wglconfig.Config, rules *GameRules, playerinfo []*ipc.GameMetadata_MinimalPlayerInfo) (*ipc.GameRecord, error) {
	...
	gstBytes, err := createGameState(cfg, rules.boardLayout, len(playerinfo), rules)
	if err != nil {
		return nil, err
	}
	...
	return &ipc.GameRecord{
		GameMetadata: gmeta,
		Events:       make([]*ipc.GameEvent, 0),
		GameState:    gstBytes,
		Version:      stores.CurrentGameRecordVersion,
	}, nil
}

func StartGame(ctx context.Context, gdoc *ipc.GameDocument, st *gamestate.GameState) error {
	if gdoc.PlayState != ipc.PlayState_UNSTARTED {
		return errStartNotPermitted
	}
	// We can pass in a sync.Pooled instance here to avoid allocations.
	// But it seems the allocation is so small (just a holding struct with a
	// couple of pointers), that we probably don't need to bother.
	// We can revisit if this actually becomes a problem.
	tilebag := st.Bag(nil)

	racks := racksFromGameState(st)

	for idx := range gdoc.Players {
		err := tiles.Draw(tilebag, RackTileLimit, racks[idx*RackTileLimit:(idx+1)*RackTileLimit])
		if err != nil {
			return err
		}
	}

	timers := st.Timers(nil)
	resetTimersAndStart(timers, globalNower)
	// Outside of this:
...
// This function does not need to be heavily optimized, since it is only called
// when racks are assigned via an API call (for annotator mode, for now)
func AssignRacks(st *gamestate.GameState, racks [][]byte, assignEmpty RackAssignBehavior) error {
	nplayers := st.NumPlayers()
	if len(racks) != int(nplayers) {
		return errors.New("racks length must match players length")
	}
	rack := new(gamestate.Rack)
	bag := st.Bag(nil)
	// Throw in existing racks.
	for i := range nplayers {
		if st.Racks(rack, int(i)) {
			if rack.NumTiles() > 0 {
				log.Debug().Interface("rack", rack.RackBytes()[:rack.NumTiles()]).Int("player", int(i)).Msg("throwing in rack for player")

				mls := tilemapping.FromByteArr(rack.RackBytes()[:rack.NumTiles()])
				tiles.PutBack(bag, mls)
				rack.MutateNumTiles(0) // will this work??
			}
		}
	}
	empties := []int{}
	for i, r := range racks {
		if len(r) == 0 {
			...
			if st.Racks(rack, i) {
				...
				err := tiles.RemoveTiles(bag, rackml)
				if err != nil {
					return err
				}
				...
				rbts := rack.RackBytes()
				for i := range rackml {
					rbts[i] = byte(rackml[i])
				}
				rack.MutateNumTiles(uint8(len(rackml)))
			}
		}
	}
	bagWillBeEmpty := false
	if tiles.InBag(bag) <= len(empties)*RackTileLimit {
	...
	if assignEmpty == AlwaysAssignEmpty ||
		...
		for _, i := range empties {
			if st.Racks(rack, i) {
				...
				drew, err := tiles.DrawAtMost(bag, RackTileLimit, placeholder)
				if err != nil {
					return err
				}
				...
				rbts := rack.RackBytes()
				for i := range drew {
					rbts[i] = byte(placeholder[i])
				}
				rack.MutateNumTiles(uint8(drew))
			}
		}
	}
	return nil
