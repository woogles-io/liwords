...
SELECT bg.id, bg.broadcast_id, bg.game_uuid, bg.division, bg.round, bg.table_number, bg.annotator_user_id, bg.claimed_at, bg.created_at, bg.stats, bg.stats_computed_at, bg.completed_at, u.username as annotator_username
...
SELECT bg.id, bg.broadcast_id, bg.game_uuid, bg.division, bg.round, bg.table_number, bg.annotator_user_id, bg.claimed_at, bg.created_at, bg.stats, bg.stats_computed_at, bg.completed_at, b.uuid as broadcast_uuid, b.slug as broadcast_slug, b.name as broadcast_name,
...
type GetBroadcastGameStatsBySlugRow struct {
	ID              int32
	BroadcastID     int32
	GameUuid        string
	Division        string
...
func (q *Queries) GetBroadcastGameStatsBySlug(ctx context.Context, slug string) ([]GetBroadcastGameStatsBySlugRow, error) {
	rows, err := q.db.Query(ctx, getBroadcastGameStatsBySlug, slug)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetBroadcastGameStatsBySlugRow
	for rows.Next() {
		var i GetBroadcastGameStatsBySlugRow
		if err := rows.Scan(
			&i.ID,
			&i.BroadcastID,
			&i.GameUuid,
