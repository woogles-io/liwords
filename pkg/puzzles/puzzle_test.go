...
func TestPuzzlesMain(t *testing.T) {
	...
	userIsCorrect, status, correctAnswer, gameId, _, _, attempts, _, _, err := SubmitAnswer(ctx, ps, puzzleUUID, PuzzlerUUID, &ipc.ClientGameplayEvent{}, false)
	is.NoErr(err)
	...
	userIsCorrect, status, correctAnswer, gameId, _, _, attempts, _, _, err = SubmitAnswer(ctx, ps, puzzleUUID, PuzzlerUUID, correctCGE, false)
	is.NoErr(err)
	is.True(correctAnswer != nil)
	is.True(gameId != "")
	...
	userIsCorrect, status, correctAnswer, gameId, _, _, _, _, _, err = SubmitAnswer(ctx, ps, puzzleUUID, PuzzlerUUID, correctCGE, false)
	is.NoErr(err)
	attempts, recordedCorrect, err = getPuzzleAttempt(ctx, pool, PuzzlerUUID, puzzleUUID)
	is.NoErr(err)
	is.True(userIsCorrect)
	...
	userIsCorrect, status, _, _, _, _, attempts, _, _, err = SubmitAnswer(ctx, ps, puzzleUUID, PuzzlerUUID, correctCGE, false)
	is.NoErr(err)
	...
	correctCGE = gameEventToClientGameplayEvent(correctAnswer)
	_, _, _, _, _, _, _, _, _, err = SubmitAnswer(ctx, ps, puzzleUUID, "incorrect uuid", correctCGE, false)
	...
	_, _, _, _, _, _, _, _, _, err = SubmitAnswer(ctx, ps, puzzleUUID, PuzzlerUUID, correctCGE, false)
	...
	userIsCorrect, status, correctAnswer, gameId, _, _, attempts, _, _, err = SubmitAnswer(ctx, ps, puzzleUUID, PuzzlerUUID, gameEventToClientGameplayEvent(correctAnswer), false)
	is.NoErr(err)
	is.True(!userIsCorrect)
	is.True(*status)
	is.True(correctAnswer == nil)
	...
	userIsCorrect, status, correctAnswer, gameId, _, _, attempts, _, _, err = SubmitAnswer(ctx, ps, puzzleUUID, PuzzlerUUID, gameEventToClientGameplayEvent(correctAnswer), true)
	is.NoErr(err)
	is.True(userIsCorrect)
	is.True(!*status)
	is.True(correctAnswer != nil)
	...
	userIsCorrect, status, correctAnswer, gameId, _, _, attempts, _, _, err = SubmitAnswer(ctx, ps, puzzleUUID, PuzzlerUUID, nil, true)
	is.NoErr(err)
	is.True(!userIsCorrect)
	is.True(!*status)
	is.True(correctAnswer != nil)
	...
	for i := 0; i < unseenPuzzles; i++ {
		...
		userIsCorrect, status, _, _, _, _, attempts, _, _, err = SubmitAnswer(ctx, ps, puzzleUUID, PuzzlerUUID, &ipc.ClientGameplayEvent{}, false)
		is.NoErr(err)
		is.True(!userIsCorrect)
		is.True(status == nil)
		is.Equal(attempts, int32(1))
	...
	for i := 0; i < totalPuzzles*10; i++ {
		puzzleUUID, err = GetNextPuzzleId(ctx, ps, PuzzlerUUID, common.DefaultGameReq.Lexicon)
		is.NoErr(err)
		_, _, _, _, _, _, _, _, _, err = SubmitAnswer(ctx, ps, puzzleUUID, PuzzlerUUID, &ipc.ClientGameplayEvent{}, false)
		is.NoErr(err)
	}
...
func TestPuzzlesPrevious(t *testing.T) {
	...
	_, _, _, _, _, _, _, _, _, err = SubmitAnswer(ctx, ps, puzzle1, PuzzlerUUID, &ipc.ClientGameplayEvent{}, true)
	is.NoErr(err)
	...
	is.NoErr(err)
	_, _, _, _, _, _, _, _, _, err = SubmitAnswer(ctx, ps, puzzle2, PuzzlerUUID, &ipc.ClientGameplayEvent{}, false)
	is.NoErr(err)
	...
	_, _, _, _, _, _, _, _, _, err = SubmitAnswer(ctx, ps, puzzle4, PuzzlerUUID, &ipc.ClientGameplayEvent{}, false)
	is.NoErr(err)
	...
	is.NoErr(err)
	_, _, _, _, _, _, _, _, _, err = SubmitAnswer(ctx, ps, puzzle5, PuzzleCreatorUUID, &ipc.ClientGameplayEvent{}, false)
	is.NoErr(err)
	for i := 0; i < 5; i++ {
		...
		is.NoErr(err)
		_, _, _, _, _, _, _, _, _, err = SubmitAnswer(ctx, ps, otherPuzzle, PuzzleCreatorUUID, &ipc.ClientGameplayEvent{}, false)
		is.NoErr(err)
	}
