...
func ReplayEvents(ctx context.Context, cfg *wglconfig.Config, gdoc *ipc.GameDocument, evts []*ipc.GameEvent, rememberRacks bool) error {
	...
	gdoc.PlayerOnTurn = 0
	// The end-rack event is regenerated by endRackCalcs; note where the last one
	// is so we can tell a full game from a replayed prefix.
	lastEndRack := -1
	...
	// The player whose placement went out, tracked as we replay. Racks are not
	// a reliable source for this mid-replay, and the events come from an
	// untrusted GCG, so take it from the play that ended the game.
	wentOut := -1
	var savedRacks [][]byte
	if rememberRacks {
	...
	for idx, evt := range evts {
		...
		default:
			...
			// as in challengeEvent; there is no final pass to replay. Only do
			// this if the end-rack event is still to come; otherwise we are
			// replaying a prefix of the game (an editor amendment) and the
			// caller is going to add more events.
			if evt.Type == ipc.GameEvent_CHALLENGE_BONUS &&
				gdoc.PlayState == ipc.PlayState_WAITING_FOR_FINAL_PASS &&
				idx < lastEndRack {
				if wentOut < 0 {
					return errors.New("waiting for final pass but no play went out")
				}
				// As in challengeEvent, the challenger is left on turn. This
				// must happen before the game is over, since the assignment is
				// skipped once it is.
				if err := assignTurnToNextNonquitter(gdoc, uint32(wentOut)); err != nil {
					return err
				}
