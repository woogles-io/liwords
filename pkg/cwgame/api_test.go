...
func TestNewGame(t *testing.T) {
	...
	g, err := NewGame(DefaultConfig.MacondoConfig.WGLConfig(), rules, []*ipc.GameDocument_MinimalPlayerInfo{
...
func TestStartGame(t *testing.T) {
	...
	rules := NewBasicGameRules("NWL20", "CrosswordGame", "english", ipc.ChallengeRule_ChallengeRule_FIVE_POINT,
		"classic", []int{300, 300}, 1, 0, false)
	g, _ := NewGame(DefaultConfig.MacondoConfig.WGLConfig(), rules, []*ipc.GameDocument_MinimalPlayerInfo{
		{Nickname: "Cesitar", RealName: "Cesar", UserId: "cesar1"},
		{Nickname: "Lucas", RealName: "Lucas", UserId: "lucas1"},
	})
...
func norwegianBytes(tiles string) []byte {
	ld, err := tilemapping.GetDistribution(DefaultConfig.MacondoConfig.WGLConfig(), "norwegian")
	if err != nil {
		panic(err)
	}
	mw, err := tilemapping.ToMachineWord(tiles, ld.TileMapping())
...
func TestProcessGameplayEventSanityChecks(t *testing.T) {
	...
	for _, tc := range testcases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			err := ProcessGameplayEvent(ctx, DefaultConfig.MacondoConfig.WGLConfig(), tc.cge, tc.userID, gdoc)
...
func TestChallengeBadWord(t *testing.T) {
	...
	err := ProcessGameplayEvent(ctx, DefaultConfig.MacondoConfig.WGLConfig(), cge, "2gJGaYnchL6LbQVTNQ6mjT", gdoc)
	is.NoErr(err)
	...
	err = ProcessGameplayEvent(ctx, DefaultConfig.MacondoConfig.WGLConfig(), cge2, "FDHvxexaC5QNMfiJnpcnUZ", gdoc)
	is.NoErr(err)
	...
	err = ProcessGameplayEvent(ctx, DefaultConfig.MacondoConfig.WGLConfig(), cge3, "FDHvxexaC5QNMfiJnpcnUZ", gdoc)
	is.NoErr(err)
...
func TestChallengeGoodWordDouble(t *testing.T) {
	...
	err = ProcessGameplayEvent(ctx, DefaultConfig.MacondoConfig.WGLConfig(), cge3, "2gJGaYnchL6LbQVTNQ6mjT", gdoc)
	is.NoErr(err)
	is.Equal(len(gdoc.Events), 7)
...
func TestChallengeGoodWordNorwegian(t *testing.T) {
	...
	err = ProcessGameplayEvent(ctx, DefaultConfig.MacondoConfig.WGLConfig(), cge, "cesar1", g)
	is.NoErr(err)
	...
	err = ProcessGameplayEvent(ctx, DefaultConfig.MacondoConfig.WGLConfig(), cge2, "lucas1", g)
	is.NoErr(err)
...
func TestLoadChallengeOrPassState(t *testing.T) {
	...
	for _, finalmove := range []ipc.ClientGameplayEvent_EventType{
		...
		t.Run(ipc.ClientGameplayEvent_EventType_name[int32(finalmove)], func(t *testing.T) {
			...
			err := ProcessGameplayEvent(ctx, DefaultConfig.MacondoConfig.WGLConfig(), cge, "FDHvxexaC5QNMfiJnpcnUZ", gdoc)
			is.NoErr(err)
...
func TestConsecutiveScorelessTurns(t *testing.T) {
	...
	err := ProcessGameplayEvent(ctx, DefaultConfig.MacondoConfig.WGLConfig(), passCGE, "2gJGaYnchL6LbQVTNQ6mjT", gdoc)
	is.NoErr(err)
	is.Equal(gdoc.ScorelessTurns, uint32(1))
	err = ProcessGameplayEvent(ctx, DefaultConfig.MacondoConfig.WGLConfig(), passCGE, "FDHvxexaC5QNMfiJnpcnUZ", gdoc)
	is.NoErr(err)
	...
	err = ProcessGameplayEvent(ctx, DefaultConfig.MacondoConfig.WGLConfig(), phonyCGE, "2gJGaYnchL6LbQVTNQ6mjT", gdoc)
	is.NoErr(err)
	is.Equal(gdoc.ScorelessTurns, uint32(0))
	err = ProcessGameplayEvent(ctx, DefaultConfig.MacondoConfig.WGLConfig(), &ipc.ClientGameplayEvent{
		Type:   ipc.ClientGameplayEvent_CHALLENGE_PLAY,
		GameId: "9aK3YgVk",
	...
	err = ProcessGameplayEvent(ctx, DefaultConfig.MacondoConfig.WGLConfig(), exchangeCGE, "2gJGaYnchL6LbQVTNQ6mjT", gdoc)
	is.NoErr(err)
	...
	err = ProcessGameplayEvent(ctx, DefaultConfig.MacondoConfig.WGLConfig(), phonyCGE, "FDHvxexaC5QNMfiJnpcnUZ", gdoc)
	is.NoErr(err)
	is.Equal(gdoc.ScorelessTurns, uint32(0))
	...
	err = ProcessGameplayEvent(ctx, DefaultConfig.MacondoConfig.WGLConfig(), chCGE, "2gJGaYnchL6LbQVTNQ6mjT", gdoc)
	is.NoErr(err)
...
func TestResign(t *testing.T) {
	...
	for _, tc := range testcases {
		...
		t.Run(fmt.Sprintf(tc.name), func(t *testing.T) {
			...
			err := ProcessGameplayEvent(ctx, DefaultConfig.MacondoConfig.WGLConfig(), resignCGE, tc.resignerid, gdoc)
			if err != nil {
				is.Equal(err, tc.expectedErr)
				return
			}
...
func TestVoidChallenge(t *testing.T) {
	...
	err = ProcessGameplayEvent(ctx, DefaultConfig.MacondoConfig.WGLConfig(), cge2, "2gJGaYnchL6LbQVTNQ6mjT", gdoc)
	is.NoErr(err)
	is.Equal(gdoc.CurrentScores, []int32{113, 137})
}
...
func TestExchange(t *testing.T) {
	...
	err := ProcessGameplayEvent(ctx, DefaultConfig.MacondoConfig.WGLConfig(), cge, userID, gdoc)
	is.NoErr(err)
	...
	err = ReconcileAllTiles(DefaultConfig.MacondoConfig.WGLConfig(), gdoc)
	is.NoErr(err)
}
...
func TestAssignRacks(t *testing.T) {
	...
	dist, err := tilemapping.GetDistribution(DefaultConfig.MacondoConfig.WGLConfig(), "English")
	is.NoErr(err)
...
func TestAssignRacksIfBagEmpty(t *testing.T) {
	...
	is.NoErr(ReconcileAllTiles(DefaultConfig.MacondoConfig.WGLConfig(), gdoc))
}
...
func TestReplayEvents(t *testing.T) {
	...
	for _, tc := range testcases {
		...
		t.Run(tc, func(t *testing.T) {
			...
			ctx := ctxForTests()
			err := ReplayEvents(ctx, config.DefaultConfig().MacondoConfig.WGLConfig(), gdoc, gdocClone.Events, true)
			is.NoErr(err)
			tiles.Sort(gdoc.Bag)
...
func TestEditOldRack(t *testing.T) {
	...
	err := EditOldRack(ctx, DefaultConfig.MacondoConfig.WGLConfig(), gdoc, 0, []byte{1, 2, 3, 4, 5, 6, 7})
	is.NoErr(err)
...
func TestEditOldRackDisallowed(t *testing.T) {
	...
	err := EditOldRack(ctx, DefaultConfig.MacondoConfig.WGLConfig(), gdoc, 8, []byte{10, 11, 12})
	is.Equal(err.Error(), "tried to remove tile 10 from bag that was not there")
	err = EditOldRack(ctx, DefaultConfig.MacondoConfig.WGLConfig(), gdoc, 7, []byte{10, 11, 12})
	is.NoErr(err)
...
func TestToCGP(t *testing.T) {
	...
	cgp, err := ToCGP(DefaultConfig.MacondoConfig.WGLConfig(), gdoc)
	is.NoErr(err)
