...
func TestStandings(t *testing.T) {
	...
	simResults, pairErr := standings.SimFactorPairAll(ctx, req, copRand, 10, int(req.ValidPlayers), false, nil)
	...
	numSims := 1000
	simResults, pairErr = standings.SimFactorPairAll(ctx, req, copRand, numSims, 2, false, nil)
	is.Equal(pairErr, pb.PairError_SUCCESS)
	...
	simResults, pairErr = standings.SimFactorPairAll(ctx, req, copRand, numSims, 3, false, nil)
	is.Equal(pairErr, pb.PairError_SUCCESS)
	...
	is.True(verifyreq.Verify(req) == nil)
	standings = pkgstnd.CreateInitialStandings(req)
	simResults, pairErr = standings.SimFactorPairAll(ctx, req, copRand, numSims, 2, true, nil)
	is.Equal(pairErr, pb.PairError_SUCCESS)
	...
	standings = pkgstnd.CreateInitialStandings(req)
	numSims = 5000
	simResults, pairErr = standings.SimFactorPairAll(ctx, req, copRand, numSims, 3, true, nil)
	is.Equal(pairErr, pb.PairError_SUCCESS)
	is.Equal(simResults.HighestControlLossRankIdx, 1)
