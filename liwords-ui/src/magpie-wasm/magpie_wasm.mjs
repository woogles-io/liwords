...
["_free","_main","_malloc","_precache_file_data","_process_ucgi_command","_score_play","__emscripten_thread_init","__emscripten_thread_exit","__emscripten_thread_crashed","__emscripten_thread_mailbox_await","__emscripten_tls_init","_pthread_self","checkMailbox","establishStackSpace","invokeEntryPoint","PThread","_fflush","__emscripten_check_mailbox","onRuntimeInitialized"].forEach((prop) => {
...
var wasmBinaryFile;
if (Module['locateFile']) {
  wasmBinaryFile = 'magpie_wasm.wasm';
  if (!isDataURI(wasmBinaryFile)) {
    wasmBinaryFile = locateFile(wasmBinaryFile);
  }
} else {
  // Use bundler-friendly `new URL(..., import.meta.url)` pattern; works in browsers too.
  wasmBinaryFile = new URL('magpie_wasm.wasm', import.meta.url).href;
}
...
// end include: preamble.js
  ...
  var PThread = {unusedWorkers:[],runningWorkers:[],tlsInitFunctions:[],pthreads:{},nextWorkerID:1,debugInit:function() {
        ...
        var pthreadPoolSize = navigator.hardwareConcurrency + 3;
        ...
        if (!Module['locateFile']) {
          worker = new Worker(new URL('magpie_wasm.worker.js', import.meta.url));
        } else {
        // Allow HTML module to configure the location where the 'worker.js' file will be loaded from,
        // via Module.locateFile() function. If not specified, then the default URL 'worker.js' relative
        // to the main html file is loaded.
        var pthreadMainJs = locateFile('magpie_wasm.worker.js');
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(4, 1, fd, cmd, varargs);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(5, 1, fd, op, varargs);
  ...
  var __emscripten_notify_mailbox_postmessage = function(targetThreadId,
                                                     currThreadId,
                                                     mainThreadId) {
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(7, 1, fd);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(9, 1, fd, offset_low, offset_high, whence, newOffset);
...
var proxiedFunctionTable = [null,_proc_exit,exitOnMainThread,pthreadCreateProxied,___syscall_fcntl64,___syscall_ioctl,___syscall_openat,_fd_close,_fd_read,_fd_seek,_fd_write];
...
/** @type {function(...*):?} */
var _precache_file_data = Module["_precache_file_data"] = createExportWrapper("precache_file_data");
/** @type {function(...*):?} */
var _fflush = Module["_fflush"] = createExportWrapper("fflush");
/** @type {function(...*):?} */
var _process_ucgi_command = Module["_process_ucgi_command"] = createExportWrapper("process_ucgi_command");
/** @type {function(...*):?} */
var _score_play = Module["_score_play"] = createExportWrapper("score_play");
/** @type {function(...*):?} */
