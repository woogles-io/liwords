...
import { DeclineSeekRequest, SeekRequest, SoughtGameProcessEvent } from '../gen/api/proto/ipc/omgseeks_pb';
    ...
    let ret = (
        <div className={`game-container${isRegistered ? ' competitor' : ''}`}>
            ...
            <div className={`game-table ${boardTheme} ${tileTheme}`}>
                <div
                    className={`chat-area ${!isExamining && tournamentContext.metadata.getDisclaimer()
                        ? 'has-disclaimer'
                        : ''
                        }`}
                    id="left-sidebar"
                >
                    ...
                    {playerNames.length > 1 ? (
                        <Chat
                            ...
                            defaultChannel={`chat.${isObserver ? 'gametv' : 'game'
                                }.${gameID}`}
