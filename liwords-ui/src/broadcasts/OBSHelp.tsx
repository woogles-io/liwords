...
/**
 * The OBS manual. One document, opened from every place a slot is set up, with
 * the caller choosing which chapter to land on.
 *
...
const C: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <code className="obs-help-code">{children}</code>
);

const P: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <Typography.Paragraph className="obs-help-p">{children}</Typography.Paragraph>
);

const H: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <Typography.Title level={5} className="obs-help-h">
    {children}
  </Typography.Title>
);
...
const Overview = () => (
  <>
    ...
    <pre className="obs-help-diagram">
  ...
  └─ stream slot            ── yours; you choose what it follows
    ...
    <table className="obs-help-table">
      <thead>
        <tr>
          <th />
...
const SECTIONS: { key: OBSHelpSection; label: string; body: React.FC }[] = [
...
export const OBSHelpButton: React.FC<Props> = ({
  ...
  const [active, setActive] = useState<OBSHelpSection>(section);
      ...
      >
        <div ref={wrapRef}>
          <Tabs
            tabPosition="left"
            activeKey={active}
            onChange={(k) => setActive(k as OBSHelpSection)}
            className="obs-help-tabs"
            items={SECTIONS.map((s) => ({
              key: s.key,
              label: s.label,
              children: (
                <div className="obs-help-body">
                  <s.body />
                </div>
              ),
            }))}
