...
import { TournamentsPage } from "./tournament/tournaments_page";
...
import {
  wooglesTheme,
  wooglesCssVariablesResolver,
} from "./theme/woogles-theme";
...
const App = React.memo(() => {
  ...
  return (
    ...
    // forceColorScheme rather than Mantine's own colour-scheme manager: the
    // Zustand store already owns the mode (and the legacy `darkMode` localStorage
    // key), and letting Mantine keep a second `mantine-color-scheme` key would
    // give us two sources of truth that can disagree.
    <MantineProvider
      theme={wooglesTheme}
      cssVariablesResolver={wooglesCssVariablesResolver}
      forceColorScheme={themeMode === "dark" ? "dark" : "light"}
    >
      <StyleProvider layer>
