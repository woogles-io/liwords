...
| 1b  | Unwrap `colorModed()`                         | `mantine-02-unwrap-colormoded` | [#1970](https://github.com/woogles-io/liwords/pull/1970) | 638 blocks across 31 files. Byte-identical compiled CSS. `m()`/`d()` kept — they validate token names at build time.                                    |
| 0b  | Mantine boot: deps, PostCSS, providers, theme | `mantine-04-mantine-boot`      | [#1979](https://github.com/woogles-io/liwords/pull/1979) | Mantine 9.5.2. Theme reads `--woogles-*` via `cssVariablesResolver`. CSS +33.5 kB gz (full `styles.layer.css`); JS +0.5 kB gz.                          |
