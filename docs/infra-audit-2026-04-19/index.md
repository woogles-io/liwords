...
10. **Use `game_players.created_at` as the authoritative `ended_at` source for backfill**, not `games.updated_at`. The latter drifts on post-end edits; recent commit churn on timestamp consistency (`27c01345b`, `fba8733c7`, `389325273`, `dee6b2d9e`, `e0b449bc0`) confirms this is not safe as a proxy. New schema writes `ended_at` explicitly at game-end transaction time.
